...
*/
 ...
 for(fr=startfr ; fr<=endfr ; )
 {
     ...
     phi = calc_phi(sobs,cobs)*180/M_PI; //(-atan(sobs/cobs)+M_PI)/(2*M_PI);
     if (phi < 0){
	 	phi += 360;
     }
	 phi = phi/360;
 ...
 */  
 ...
 /* Writing peaks */
 //lmy if(ipeak) printf("   f[1/d]      P[d]      Amp      Phase        S/N （ipeak num=%d)\n", ipeak);
 //if(ipeak) printf("   f[1/d]      P[d]      Amp      Phase        S/N （ipeak num=%d)\n", ipeak);
 for(i=0 ; i<ipeak ; i++)
   {
      peak[i]/=2.0*M_PI ;
