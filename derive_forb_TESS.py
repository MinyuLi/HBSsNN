...
unit = 1000
sn_level=4.
low_limit_f = 0.037 #0.037 corresponds to 27 days. if there are more than 2 sectors, this value should be adjusted.
...
def toSpectra(lc_path, maxf):
	...
	command = fnpeaks + ' -f ' + '\'' + lc_path + '\'' + ' 0 '+strMaxf+' 0.00025'
	...
	for i in range(10):
		for k in range(len(f0)):
			...
			for j in range(len(valid_freqs)):
				...
				if harmonic > 0 and harmonic not in harmonic_lst:
...
def calc_forb(freq_lst, amp_lst, f0,freq_resolution):
	...
	judge_reso = 0.005
	freqs_intervals = freqs_intervals[freqs_intervals>low_limit_f]
...
def get_freq_cluster(freq1):
	...
	if len(freq_cluster) > 5:
		freq_cluster = freq_cluster[0:5]
	return freq_cluster
...
def derive_forb(obj_id, lc_path, result_path):
	...
	trf_file, max_file = toSpectra(lc_path, 5)
